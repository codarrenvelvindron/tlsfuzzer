...
def help_msg():
    ...
    print(" -i ext-id      exclude specific extenion id from a list")
...
def main():
    ...
    argv = sys.argv[1:]
    opts, args = getopt.getopt(argv, "h:p:e:n:i:", ["help"])
    for opt, arg in opts:
        if opt == '-h':
            host = arg
        elif opt == '-p':
    ...
    unassinged_ext_id = list(range(2, 5))
    unassinged_ext_id += list(range(6, 10))
    unassinged_ext_id += [11, 12, 17]
    unassinged_ext_id += list(range(22, 41))
    unassinged_ext_id += [46]
    unassinged_ext_id += list(range(52, 65536))

    # Exclude extensions from a list of unassigned ones
    unassinged_ext_id = list(filter(lambda x: x not in ext_exclude, unassinged_ext_id))

    number_of_chunks = 5
    chunk_size = int(len(unassinged_ext_id) / number_of_chunks)
    for ext_chunk in [unassinged_ext_id[j:j + chunk_size]
                      for j in range(0, len(unassinged_ext_id), chunk_size)]:
        conversation = Connect(host, port)
        node = conversation
        ciphers = [CipherSuite.TLS_AES_128_GCM_SHA256,
                CipherSuite.TLS_EMPTY_RENEGOTIATION_INFO_SCSV]
        ext = {ext_id: AutoEmptyExtension() for ext_id in ext_chunk}
        groups = [GroupName.secp256r1]
        key_shares = []
        for group in groups:
            key_shares.append(key_share_gen(group))
